...
/**
 * Checks the global firmware update status (g_fw_update_status) and creates the fw_update_reset timer if g_fw_update_status is true.
*/
static void http_server_fw_update_reset_timer(void)
{
    if (g_fw_update_status == OTA_UPDATE_SUCCESSFUL)
    {
        ...
        // Give the webpage a chance to receive an acknowledge back and initialize the timer
...
{
    ESP_LOGI(TAG, "flavicon.ico requested");
...
esp_err_t http_server_OTA_update_handler(httpd_req_t *req)
{
    ...
    do
    {
        ...
        if (!is_req_body_started)
        {
            ...
            char *body_start_p = strstr(ota_buff, BODY_START) + strlen(BODY_START);
    ...
    {
        ESP_LOGI(TAG, "http_server_OTA_update_handler: esp_ota_end!!!");
    }

    // We won't update the global variables throughout the file, so lets send the message about the status
    if (flash_successful)
    {
...
/**
 * wifiConnectionStatus handler updates the connection status for the web page.
 * @param req HTTP request for which the uri needs to be handled.
 * @return ESP_OK 
*/
...
/**
 * wifiConnectionInfo handler updates the web page with connection information.
 * @param req HTTP request for which the uri needs to be handled.
 * @return ESP_OK 
*/
...
/**
 * wifiDisconnection.json handler responds by sending a message to the WiFi application to disconnect.
 * @param req HTTP request for which the uri needs to be handled.
 * @return ESP_OK 
*/
...
static httpd_handle_t http_server_configure(void)
{
    ...
    if (httpd_start(&http_server_handle, &config) == ESP_OK)
    {
        ...
        // register wifiDisconnectInfo.json handler
